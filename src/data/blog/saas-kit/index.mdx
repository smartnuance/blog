...
This kit gives you a jump start for your next SaaS backend. It combines the benefits of a streamlined developer experience familiar from monolithic frameworks with the design advantages of microservices (checkout ["Soft factors of microservices"](/blog/microservices)).
...
- [Authentication service](#token-authentication) with a hierarchical role model for organizing permissions
...
- We add a many-to-many relation from _User_ to _Instance_, over the intermediate table _Profile_. A user might have profiles for several instances with different roles.
